...
int main(int argc, char* argv[])
{
    ...
    */
    try
    {
        ...
        into a TCP endpoint. To do this we use a ip::tcp::resolver object.
        */
        ...
        */
        ...
        /*
        The list of endpoints is returned using an iterator of type ip::tcp::resolver::iterator (note that a default constructed iterator object can be used as an end iterator).
        */
        tcp::resolver::iterator endpoint_iterator = resolver.resolve(query);

        /*
        Now we create and connect the socket. The list of endpoints obtained above may contain both IPv4 and IPv6 endpoints, so we need to try each of them until we find one that works. This keeps the client program indepdendent of a specific IP version. The boost::asio::connect() function does this for us automatically.
        */
